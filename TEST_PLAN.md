...
> نسخهٔ Phase 3 — آزمون L1 نمونهٔ مصوب M08 اجرا شده و برای Approval Gate 2 ثبت شده است.
...
## خط مبنای Gate 1 و نتیجهٔ Phase 3
...
| پوشش نمونهٔ M08 | `npm run test:content` | Pass — ۲۹/۲۹ مفهوم؛ ۲۸/۲۸ کنترل |
| وب responsive و RTL | `npm run test:web` | Pass — ۷۱/۷۱ کنترل در Chromium و Firefox |
| PDF رسمی | `npm run test:pdf` | Pass — ۲۵/۲۵ کنترل؛ ۲۵ صفحهٔ A4 |
| بازبینی دیداری | screenshotهای وب و ۷ contact sheet از تمام صفحات PDF | Pass — بدون clipping، overlap یا صفحهٔ سفید |
...
## نتیجهٔ امضاشدهٔ L1 در Phase 3

| حوزه | شاهد | نتیجه |
|---|---|---|
| محتوا و پوشش | `artifacts/qa/phase3/content-audit.json` | ۲۸ pass، صفر fail؛ ۱۹۶ ID یکتا و ۲۹ ID مربوط به M08 با anchor واقعی |
...
| رسانه | همان گزارش و `MEDIA_SOURCES.md` | شش SVG اصیل، شش manifest entry و checksum منطبق؛ صفر تصویر مقالهٔ مبدأ |
| وب | `artifacts/qa/phase3/web-audit.json` | ۷۱ pass، صفر fail؛ پنج profile در Chromium و پنج profile در Firefox |
| دسترس‌پذیری وب | همان گزارش | صفر violation جدی/بحرانی axe؛ keyboard، footnote، alt و JS-off عبور کردند |
| PDF | `artifacts/qa/phase3/pdf-audit.json` | ۲۵ pass، صفر fail؛ ۲۵ صفحه، A4، مقصدهای داخلی و ۳۰ رکورد فونت همگی embedded/subset |
| رندر PDF | هفت `pdf-contact-*.png` | تمام ۲۵ صفحه با Poppler رندر، decode و صفحه‌به‌صفحه بازبینی شدند |
| بازتولیدپذیری | clone تمیز + `npm ci` + bootstrap checksum‌شده + build | `diff -qr` وب و `cmp` PDF بدون تفاوت؛ PDF با SHA-256 یکسان |
| مخزن | `git diff --check` و کنترل syntax | بدون خطای whitespace و بدون دستور ساخت فرضی |

Chromium دقیقاً در عرض‌های ۳۶۰، ۳۹۰، ۷۶۸، ۱۲۸۰ و ۱۴۴۰ پیکسل اجرا شد. Selenium/Firefox در این محیط برای دو درخواست موبایل حداقل viewport داخلی ۴۳۸ پیکسل ایجاد کرد؛ نتیجهٔ Firefox برای RTL، font، image و overflow در عرض واقعی ۴۳۸ ثبت شده و پوشش دقیق ۳۶۰/۳۹۰ را Chromium فراهم می‌کند. WebKit مطابق برنامه به L3 موکول است.
...
- ماژول نمونه coverage کامل خود را دارد؛ web/PDF build از QMD مشترک؛ تست‌های L1 pass؛ defectهای RTL/PDF و تصمیم‌های stack ثبت. **وضعیت فنی: آمادهٔ تصمیم؛ تصویب صریح هنوز صادر نشده است.**
